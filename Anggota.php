...
<body class="bg-background-light dark:bg-background-dark font-display text-slate-900 dark:text-white antialiased min-h-screen flex flex-col">
        ...
        <div class="max-w-[1920px] mx-auto px-6 lg:px-12">
            <div class="flex h-20 items-center justify-between">
                ...
                <nav class="hidden md:flex items-center gap-1">
                    ...
                    <a class="px-5 py-2.5 text-sm font-semibold text-primary bg-primary/5 rounded-full transition-all duration-300" href="Anggota.php">
                        Anggota
                    </a>
    ...
    <main class="flex-grow p-6 lg:p-12 relative overflow-hidden">
        ...
        <div class="max-w-[1600px] mx-auto relative z-10">
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
                <div>
                    <h2 class="text-3xl font-bold text-slate-900 dark:text-white">Manajemen Anggota</h2>
                    <p class="text-slate-500 dark:text-slate-400 mt-1">Kelola data seluruh anggota perpustakaan</p>
                </div>
                <div class="flex flex-col sm:flex-row gap-4 items-center">
                    <form action="Anggota.php" method="GET" class="flex flex-col sm:flex-row gap-4 items-center">
                        <div class="relative w-full sm:w-80">
                        <span class="material-symbols-outlined absolute left-4 top-1/2 -translate-y-1/2 text-slate-400">search</span>
                        <input name="cari" class="glass-search w-full pl-12 pr-4 py-3 rounded-xl border-none ring-1 ring-slate-200 dark:ring-slate-700 focus:ring-2 focus:ring-primary transition-all outline-none text-slate-700 dark:text-slate-200" placeholder="Cari nama atau NIS..." type="text" />
                    </div>
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="bg-slate-50/50 dark:bg-slate-800/50 border-b border-slate-100 dark:border-slate-800">
                                ...
                                <th class="px-8 py-5 text-sm font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">NIS</th>
                                <th class="px-8 py-5 text-sm font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Alamat</th>
                                <th class="px-8 py-5 text-sm font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">NO HP</th>
                        ...
                        <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                            <tr class="group hover:bg-slate-50/80 dark:hover:bg-slate-800/30 transition-colors duration-150">
                                <td class="px-8 py-5 text-sm font-medium text-primary"><?= $tampil['ID_Anggota'] ?></td>
                                <td class="px-8 py-5">
                                    <div class="flex items-center gap-3">
                                        <span class="font-semibold text-slate-900 dark:text-white"><?= $tampil['Nama']?></span>
                                    </div>
                                </td>
                                <td class="px-8 py-5 text-sm text-slate-600 dark:text-slate-300"><?= $tampil['NIS']?></td>
                                <td class="px-8 py-5 text-sm text-slate-600 dark:text-slate-300"><?= $tampil['Alamat']?></td>
                                <td class="px-8 py-5 text-sm text-slate-600 dark:text-slate-300"><?= $tampil['No_hp']?></td>
                                <td class="px-8 py-5 text-right">
                                    <div class="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                        <button onclick="openEditModal('<?=$tampil['ID_Anggota']?>', '<?=$tampil['Nama']?>', '<?=$tampil['NIS']?>', '<?=$tampil['Alamat']?>', '<?=$tampil['No_hp']?>')"
                                            class="p-2 text-slate-400 hover:text-primary hover:bg-primary/5 rounded-lg transition-colors">
                                            <span class="material-symbols-outlined text-[20px]">edit</span>
                                        </button>
                                        <a href="CRUD_Delete/Delete_Anggota.php?id=<?= $tampil['ID_Anggota'];?>"
                                            onclick="return confirm(`Apakah Anda yakin akan menghapus anggota ini?`)" 
    ...
    <div class="flex min-h-full items-center justify-center p-4">
        <div class="relative w-full max-w-md transform overflow-hidden rounded-2xl bg-white dark:bg-[#1e2330] p-8 shadow-2xl transition-all animate-fade-in-up">
            ...
            <form action="CRUD_TAMBAH\Tambah_Anggota.php" method="POST" class="space-y-6">
                <div>
                    <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">ID ANGGOTA</label>
                    <input type="text" placeholder="sementara doang" name="ID" class="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-transparent focus:ring-2 focus:ring-primary focus:border-transparent outline-none transition-all dark:text-white">
                </div>
                <div>
                ...
                <div>
                    <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">NIS (Nomor Induk Siswa)</label>
                    <input type="text" placeholder="Masukkan NIS..." name="NIS" class="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-transparent focus:ring-2 focus:ring-primary focus:border-transparent outline-none transition-all dark:text-white">
                </div>
                ...
                <div>
                    ...
                    <input type="text" placeholder="Masukan No HP 08..." name="no_hp" class="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-transparent focus:ring-2 focus:ring-primary focus:border-transparent outline-none transition-all dark:text-white">
                </div>
    ...
    <div class="relative w-full max-w-lg transform overflow-hidden rounded-2xl bg-white dark:bg-[#1e2330] p-8 shadow-2xl">
        ...
        <form action="CRUD_Edit/Edit_Anggota.php" method="POST" class="space-y-4">
            <div>
                <label class="block text-sm font-semibold mb-2">ID_Anggota</label>
                <input type="text" id="edit_id" name="id" readonly class="w-full px-4 py-3 rounded-xl bg-slate-100 dark:bg-slate-800 border-none outline-none">
            </div>
            <div>
            ...
            <div>
                ...
                <input type="text" id="edit_nis" name="nis" class="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-transparent">
            </div>
            <div>
            ...
            <div>
                ...
                <input type="text" id="edit_no_hp" name="no_hp" class="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-transparent">
            </div>
